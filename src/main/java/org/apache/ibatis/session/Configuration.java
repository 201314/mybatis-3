...
public class Configuration {
   ...
   *
   * @param transaction
   * @param executorType 默认就是 ExecutorType.SIMPLE
   * @return
   */
  public Executor newExecutor(Transaction transaction, ExecutorType executorType) {
    ...
    /**
     * 二级缓存默认开启 , 最终由 TransactionalCache 来缓存查询结果，但是不建议开启，
     * 因为缓存由一个Map来存储，且没有容量限制
     *
     * 建议直接使用Redis来缓存即可，数据库只要命中索引慢不了
     *
     * 不过此处可以了解下，生成cacheKey的机制
     */
    if (cacheEnabled) {
